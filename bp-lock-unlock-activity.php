<?php
/**
 * Plugin Name: BuddyPress Lock Unlock-Aktivität
 * Version: 1.1.8
 * Plugin URI: https://n3rds.work/docs/buddypress-lock-unlock-aktivitaet-handbuch/
 * Author: DerN3rd
 * Author URI: https://n3rds.work
 * Description: Benutzern erlauben, Kommentare für Aktivitäten zu sperren.
 *
 * Original Contributor: DerN3rd
 */

require 'psource-plugin-update/plugin-update-checker.php';
$MyUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	'https://n3rds.work//wp-update-server/?action=get_metadata&slug=bp-lock-unlock-activity', 
	__FILE__, 
	'bp-lock-unlock-activity' 
);
...
class BP_Lock_Unlock_Activity_Helper {
		...
		if ( ! bp_is_user() && bp_is_activity_component() && bp_current_action() && ! bp_is_single_activity() ) {
			...
			if ( self::user_can_update_activity( $activity_id ) ) {
				...
				if ( 'close' === $action ) {
					...
					$message = __( 'Aktivität zum Kommentieren gesperrt.', 'bp-lock-unlock-activity' );
				} elseif ( 'open' === $action ) {
					// Are we opening again the activity for commenting.
					self::open( $activity_id );
					$message = __( 'Aktivität zum Kommentieren freigeschaltet.', 'bp-lock-unlock-activity' );
				}
			} else {
				// let user know that he is a crook may be.
				$message = __( "Du hast keine Berechtigung dazu.", 'bp-lock-unlock-activity' );
			}
	...
	public function ajax_lock_unlock_activity() {
		...
		if ( ! wp_verify_nonce( $_POST['_wpnonce'], 'bp_activity_open_close_link' ) ) {
			wp_send_json_error( __( 'Ungültige Aktion!', 'bp-lock-unlock-activity' ) );
		}
		...
		if ( ! self::user_can_update_activity( $activity_id ) ) {
			wp_send_json_error( __( 'Du darfst keine Aktivitäten sperren/entsperren.', 'bp-lock-unlock-activity' ) );
		}
	...
	public function get_close_open_link() {
		...
		if ( self::is_closed( $activity->id ) ) {
			$label           = __( 'Offen', 'bp-lock-unlock-activity' );
			$link_title_attr = __( 'Aktivität zum Kommentieren wieder öffnen', 'bp-lock-unlock-activity' );
			$url             = $url . 'open/' . $activity->id;
			$class           .= ' bplua-open-activity';
		} else {
			$label           = __( 'Geschlossen', 'bp-lock-unlock-activity' );
			$link_title_attr = __( 'Aktivität sperren, Kommentare nicht zulassen', 'bp-lock-unlock-activity' );
